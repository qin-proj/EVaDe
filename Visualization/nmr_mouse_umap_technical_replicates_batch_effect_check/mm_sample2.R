###############################################################################
# Description:
#This script processes scRNA-seq data from two different technical replicates of mouse bone marrow sample GSM6605113_mouse.BM.M_1. After merging the datasets, standard Seurat workflow steps are performed, culminating in UMAP visualization to examine batch effects between the technical replicates.
###############################################################################
library(Seurat)
library(tidyverse)
setwd('bm_data/')
library(Matrix)
library(dplyr)
barcode.path <- "GSM6605113_mouse.BM.M_1_1_barcodes.tsv.gz"
features.path <- "GSM6605113_mouse.BM.M_1_1_genes.tsv.gz"
matrix.path <- "GSM6605113_mouse.BM.M_1_1_matrix.mtx.gz"
...
new_cell_names <- paste0(current_cell_names, "_mouse.BM.M_1.1")
...
barcode.path <- "GSM6605114_mouse.BM.M_1_2_barcodes.tsv.gz"
features.path <- "GSM6605114_mouse.BM.M_1_2_genes.tsv.gz"
matrix.path <- "GSM6605114_mouse.BM.M_1_2_matrix.mtx.gz"
mat <- readMM(file = matrix.path)
feature.names <- read.table(features.path, header = FALSE, stringsAsFactors = FALSE)
barcode.names <- read.table(barcode.path, header = FALSE, stringsAsFactors = FALSE)
colnames(mat) <- barcode.names$V1
...
new_cell_names <- paste0(current_cell_names, "_mouse.BM.M_1.2")
...
combined_plot <- p1 + p2
ggsave("mm3_combined_umap.png", combined_plot, width = 16, height = 8, dpi = 300)
saveRDS(combined,"mouse3.rds")
