###############################################################################
# Description:
#This script processes scRNA-seq data from two technical replicates of naked mole-rat bone marrow sample GSM6605107_nmr.BM.M_2. After merging the datasets, standard Seurat workflow steps are performed, culminating in UMAP visualization to examine batch effects between the technical replicates.
###############################################################################
library(Seurat)
library(tidyverse)
setwd('bm_data/')
library(Matrix)
library(dplyr)
barcode.path <- "GSM6605107_nmr.BM.M_2_1_barcodes.tsv.gz"
features.path <- "GSM6605107_nmr.BM.M_2_1_genes.tsv.gz"
matrix.path <- "GSM6605107_nmr.BM.M_2_1_matrix.mtx.gz"
...
new_cell_names <- paste0(current_cell_names, "_nmr.BM.M_2.1")
...
barcode.path <- "GSM6605108_nmr.BM.M_2_2_barcodes.tsv.gz"
features.path <- "GSM6605108_nmr.BM.M_2_2_genes.tsv.gz"
matrix.path <- "GSM6605108_nmr.BM.M_2_2_matrix.mtx.gz"
mat <- readMM(file = matrix.path)
feature.names <- read.table(features.path, header = FALSE, stringsAsFactors = FALSE)
barcode.names <- read.table(barcode.path, header = FALSE, stringsAsFactors = FALSE)
colnames(mat) <- barcode.names$V1
...
new_cell_names <- paste0(current_cell_names, "_nmr.BM.M_2.2")
...
combined_plot <- p1 + p2
ggsave("4combined_umap.png", combined_plot, width = 16, height = 8, dpi = 300)
saveRDS(combined,"nmr4.rds")
