library(dplyr)
g01 <- read.csv("gogene_H_allpvalue.csv",header=T)
pval1 <- g01[,'p1']
...
#write.csv(filtered_rows,file="key-genes-pv",row.names=F)
write.csv(filtered_rows,file="key-genes-fdr",row.names=F)
